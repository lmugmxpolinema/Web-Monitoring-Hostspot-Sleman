...
def load_json_file(filepath: Path):
    ...
    try:
        ...
        print(f"File {filepath.name} tidak ditemukan")
        return []
    except json.JSONDecodeError as e:
        print(f"Error parsing JSON dari {filepath.name}: {e}")
        return []
...
def save_json_file(filepath: Path, data):
    ...
    try:
        ...
        print(f"Error menyimpan ke {filepath}: {e}")
...
def main():
    ...
    backup_path = BACKUP_DIR / f'onts-backup-{timestamp}.json'
    save_json_file(backup_path, existing_data)
    
    # Simpan data yang sudah digabung
    print("\n7. Menyimpan data yang sudah digabung...")
    save_json_file(ONT_FILE, merged_data)
    
    print(f"\n=== Selesai ===")
    print(f"Backup data lama: {backup_path.name}")
    print(f"Data baru tersimpan di: {ONT_FILE.name}")
